...
public class IndividualReceiptPanel extends JPanel implements ParentPanel {
    ...
    public IndividualReceiptPanel(BasePanel displayPanel) {
        ...
        String[] reasons = { "Monthly Payment of", "Celebration dues of", "Death of", "Utitlity Payment for",
                "Buying of", "Other" };
            ...
            public void actionPerformed(ActionEvent e) {
                if (checkBeforeInsert()) {
                    ...
                    if (insertionError) {
                        ...
                        customTrigger();
                        JOptionPane.showMessageDialog(displayPanel, "Saved Successfully!", "Save Receipt",
    ...
    public void prepareToShow(String ReceiptNo) {
        Object[] receiptInfo = { LocalDate.of(2023, 9, 9), ReceiptNo, "0102", "Abebe", 5000.0,
                "Monthly payment for Sep 2023", "Income", "0321", "Kebede" };
        try (Statement retrieveReceiptStmt = App.DATABASE_CONNECTION.createStatement()) {
            ResultSet retrieveReceipt = retrieveReceiptStmt.executeQuery(
                    "SELECT r.Issued_Date, LPAD(r.Receipt_No, 6, '0'), LPAD(r.Issued_For, 4, '0') AS Formatted_Issued_For, CONCAT(fi.First_Name, ' ', fi.Father_Name, ' ', fi.Grandfather_Name) AS Issuer_Name,  "
                            + //
            ...
            while (retrieveReceipt.next()) {
                for (int i = 1; i <= receiptInfo.length; i++) {
                    if (i == 1) {
                        receiptInfo[i - 1] = retrieveReceipt.getDate(i).toLocalDate();
                        continue;
                    }
                    if (i == 5) {
                        receiptInfo[i - 1] = retrieveReceipt.getDouble(i);
                        continue;
                    }
        ...
        date.getInfoLabel()
                .setText(((LocalDate) receiptInfo[0]).format(DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM)));
    ...
    private class comboListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (selected.equals("Monthly Payment of")) {
                ...
                String[] families = { "Mother", "Father", "Spouse", "Son", "Daughter", "Brother", "Sister",
    ...
    private void setNextAvailableReceipt() {
        try {
            ...
            ResultSet startingDate = nextAvailableReceiptStmt
                    .executeQuery("select LPAD(max(Receipt_No) + 1, 6, '0') AS nextAvailableReceiptNo FROM RECEIPT;");
    ...
    private void customTrigger() {
        ...
        if (reason.getSelectedItem().equals("Monthly Payment of")) {
            ...
            if (mon.equals("Dec"))
                mon = '`' + mon + '`';
            query += (mon + " = " + amount.getTextField().getText() + " WHERE ID = "
                    + issuedForID.getTextField().getText() + " AND yr = " + chooseYear.getSelectedItem() + ";");
            ...
            try (Statement updateProperties = App.DATABASE_CONNECTION.createStatement()) {
                ...
                String individualPrice = String.format("%.2f", Double.parseDouble(amount.getTextField().getText())
                        / Double.parseDouble(quantity.getTextField().getText()));
                if (ctr == 0) {
                    updateProperties
                            .executeUpdate("INSERT INTO PROPERTY(Property_Type, Number_Of_Items, Individual_Price) " + //
                                    "VALUES ('" + extraReason.getSelectedItem() + "', "
                                    + quantity.getTextField().getText() + ", " + individualPrice + ");");
                } else if (ctr > 0) {
                    updateProperties.executeUpdate("UPDATE PROPERTY " + //
                            "SET Number_Of_Items = Number_Of_Items + " + quantity.getTextField().getText()
    ...
    private void deleteReceipt() {
        try (Statement deleteStmt = App.DATABASE_CONNECTION.createStatement()) {
            deleteStmt.executeUpdate("UPDATE RECEIPT " + //
                    "SET Deleted = '" + LocalDate.now().format(DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM))
                    + " - " + deleteReason + "' " + //
            ...
            if (rsn[0].equals("Monthly")) {
                ...
                if (mon.equals("Dec"))
                    mon = '`' + mon + '`';
                query += (mon + " = " + "0" + " WHERE ID = " + issuedForID.getTextField().getText() + " AND yr = "
                        + rsn[4] + ";");
