...
public class App {
    ...
    App(String title) {
        // This block of code is to make the whole GUI look and feel like the operating
        // system. Can be seen in the ScrollPane and the file choosers
        try {
        ...
        TransparentButton HOME = new TransparentButton("Home", ImageIcons.reSize(ImageIcons.HOMEdark, 25, 25),
                window.getMenu());
        ...
        TransparentButton MEMBERS = new TransparentButton("Members", ImageIcons.reSize(ImageIcons.CONTACTSdark, 25, 25),
                window.getMenu());
        ...
        TransparentButton OFFICIAL = new TransparentButton("Officials",
                ImageIcons.reSize(ImageIcons.OFFICIALdark, 25, 25), window.getMenu());
        OFFICIAL.setSelectedIcon(ImageIcons.reSize(ImageIcons.OFFICIAL, 30, 30));
        window.getMenu().addTab(OFFICIAL, offcialsPage);
        window.getMenu().add(Box.createVerticalStrut(5));

        TransparentButton FINANCIALdata = new TransparentButton("Financial data",
                ImageIcons.reSize(ImageIcons.COINSdark, 23, 23), window.getMenu());
        ...
        TransparentButton AGENDA = new TransparentButton("Agendas", ImageIcons.reSize(ImageIcons.NOTEdark, 25, 25),
                window.getMenu());
        ...
        TransparentButton RULE = new TransparentButton("Rules", ImageIcons.reSize(ImageIcons.RULESdark, 25, 25),
                window.getMenu());
        ...
        TransparentButton HELP = new TransparentButton("Help", ImageIcons.reSize(ImageIcons.HELPdark, 27, 27),
                window.getMenu());
        HELP.setSelectedIcon(ImageIcons.reSize(ImageIcons.HELP, 32, 32));
        window.getMenu().addTab(HELP, new Help(window.getBasePanel()));
        window.getMenu().add(Box.createVerticalStrut(5));

        window.getMenu().setPreferredSize(new Dimension(140, window.getMenu().getPreferredSize().height)); // set the
                                                                                                           // size of
                                                                                                           // the
                                                                                                           // MenuBar so
                                                                                                           // as to not
        ...
        for (Component c : components) {
            try {
                ((JButton) c).setMargin(new Insets(0, 6, 0, 0));
            } catch (Exception e) {
            }
        }
